...
$output = [System.IO.Path]::GetFullPath((Join-Path $temporaryRoot ("hcop-calculator-workspace-helpers-tests-" + [guid]::NewGuid().ToString('N'))))
...
try {
  & $compiler -p (Join-Path $PSScriptRoot 'tsconfig.calculator-workspace-helpers-tests.json') --outDir $output
  if ($LASTEXITCODE -ne 0) { throw "TypeScript finalizo con codigo $LASTEXITCODE." }
  & node (Join-Path $output 'calculator-workspace.helpers.tests.js')
