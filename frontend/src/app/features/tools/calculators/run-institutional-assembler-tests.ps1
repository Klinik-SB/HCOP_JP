...
$output = [System.IO.Path]::GetFullPath((Join-Path $temporaryRoot ("hcop-institutional-assembler-tests-" + [guid]::NewGuid().ToString('N'))))
...
try {
  & $compiler -p (Join-Path $PSScriptRoot 'tsconfig.institutional-assembler-tests.json') --outDir $output
  if ($LASTEXITCODE -ne 0) { throw "TypeScript finalizo con codigo $LASTEXITCODE." }
  & node (Join-Path $output 'institutional-calculator-catalog.assembler.tests.js')
