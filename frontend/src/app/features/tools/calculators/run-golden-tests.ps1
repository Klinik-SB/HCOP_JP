...
$output = [System.IO.Path]::GetFullPath((Join-Path $temporaryRoot ("hcop-calculator-tests-" + [guid]::NewGuid().ToString('N'))))
...
try {
  & $compiler -p (Join-Path $PSScriptRoot 'tsconfig.golden.json') --outDir $output
  if ($LASTEXITCODE -ne 0) { throw "TypeScript finalizó con código $LASTEXITCODE." }
  & node (Join-Path $output 'calculator.golden-tests.js')
